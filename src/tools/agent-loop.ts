...
export async function runWithTools(prompt: string, systemPrompt?: string, tools?: ToolDefinition[]): Promise<string> {
    ...
    for (let interation = 0; interation < MAX_ITERATIONS; interation++) {
        console.log(`nPensando... (iteración ${interation + 1})`);
        ...
        if (response.stop_reason === 'end_turn') {
            const finalText = response.content.filter((block): block is Anthropic.Messages.TextBlock => block.type === "text")
        ...
        console.warn(`Stop reason inesperado: ${response.stop_reason ?? "desconocido"}`);
        const consolidateMessageText = response.content.filter((block): block is Anthropic.Messages.TextBlock =>
            block.type === "text",
        ).map((block) => block.text)
            .join("\n");

        return consolidateMessageText || `Sesión terminada: ${response.stop_reason ?? "razón desconocida"}`
    }

    console.warn(` Límite de ${MAX_ITERATIONS} iteraciones aclcanzado`);
    return (
        `Lo siento, no pude completar la tarea en ${MAX_ITERATIONS} interaciones. ` +
