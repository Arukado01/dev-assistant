...
export function validateConfig(): void{
    ...
    if(config.provider === "anthropic" && !config.anthropicApiKey) throw new Error("ANTHROPIC_MODEL está vacía. Agregala en tu archivo .env");
    
    if(config.provider === "openai" && !config.openaiApiKey) throw new Error("OPENAI_MODEL está vacía. Agregala en tu archivo .env");
